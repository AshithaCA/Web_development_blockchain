...
def police_registration():
	if 'register' in request.form:
		...
		if res:
			...
			ids=insert(q)
			qq="INSERT INTO police_station values(NULL,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"%(ids,reg_num,zone,dis,city,pin,st_name,addr,email,ph1,ph2)
			insert(qq)
...
def court_reg():
	if 'reg' in request.form:
		...
		if res:
			...
			ids=insert(q1)
			q2="INSERT INTO court VALUES(NULL,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"%(ids,crt_name,reg_num,ty_court,state,dist,addr,pin,mail,of_phone,al_phone,of_name)
			print(q2)
			insert(q2)
			flash("REGISTER SUCCESSFULLY")
			return redirect(url_for("public.log_form"))
