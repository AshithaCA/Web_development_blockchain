...
def stff_view_allocated_cases():
	...
	if action=="complete":
		q="update add_crime set status='COLLECTION COMPLETED' where crime_num='%s'"%(cid)
		update(q)
		q="update assign_staff set status='COLLECTION COMPLETED' where assign_id='%s'"%(aid)
		update(q)
		flash("updated....!")
...
def digitalevi():
	...
	if 'submit' in request.form:
		...
		return redirect(url_for('staff.stff_evi_btn',cid=cid))
